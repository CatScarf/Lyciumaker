...
import { buildSlots } from "_@vue_compiler-core@3.2.37@@vue/compiler-core";
...
export function drawText(ctx: CanvasRenderingContext2D, fg: Fragment, width: number, isMouseDown: boolean, margin: number) {
    ...
    tempCtx.font = width + "px JinMeiMaoCaoXing"
...
// 绘制提示信息
export function drawCursorPosition(ctx: CanvasRenderingContext2D, logicSize: number[], relx: number, rely: number, isMouseDown: boolean, box: Box) {
    ctx.fillStyle = "gray"
    ctx.textBaseline = "bottom"
    const text = String('cur:(' + Math.round(relx)) +
      ',' + String(Math.round(rely)) +
      ') down:' + isMouseDown +
      ' moving:' + box.moving + 
      ' startC:' + box.startC + 
      ' anchors:' + box.mkAnchors[2]

...
// 移动变形框
export function moveTransBox(box: Box, _size: number[], relx: number, rely: number, isMouseDown: boolean, isMv: boolean, width: number, keepRatio = true) {
    // 判断是否移动
    ...
    if (isMouseDown && box.moving == -1) {
        if (isBetween(relx, anchors[0][0], anchors[2][0]) && isBetween(rely, anchors[0][1], anchors[6][1])) {
            box.moving = 8;
            box.startC = [relx, rely];
            box.startSize = [size[0], size[1], size[2], size[3]]
        }

        for (let i = 0; i < anchors.length; i++) {
            if (euclideanDistance([relx, rely], anchors[i]) <= box.anchorWidth * 4) {
                box.moving = i;
                box.startC = [relx, rely];
                box.startSize = [size[0], size[1], size[2], size[3]]
                break;
    ...
    } else if (!isMouseDown && box.moving != -1) {
    ...
    // 移动
    if (box.moving != -1) {
        const speedX = relx - box.startC[0];
        const speedY = rely - box.startC[1];
